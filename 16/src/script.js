...
const graveMateial = new THREE.MeshStandardMaterial({ color: '#b2b6b1' })
for (let i = 0; i < 50; i++) {
    const angle = 2 * Math.PI * Math.random()
    const radius = Math.random() * 6 + 3
    const x = Math.sin(angle) * radius
    const z = Math.cos(angle) * radius
    const grave = new THREE.Mesh(graveGeometry, graveMateial)
    grave.position.set(x, 0.6 * 0.5, z)
...
// Door Light
const doorLight = new THREE.PointLight('#ff7d46', 17, 7)
